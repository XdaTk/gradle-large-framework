...
import cloud.tenon.gradle.bom.library.Group;
import cloud.tenon.gradle.bom.library.Library;
import cloud.tenon.gradle.maven.DeployedPlugin;
import cloud.tenon.gradle.maven.RepositoryPlugin;
...
public class BomPlugin implements Plugin<Project> {
    ...
    public void apply(Project project) {
        ...
        plugins.apply(RepositoryPlugin.class);
        plugins.apply(JavaPlatformPlugin.class);

        JavaPlatformExtension javaPlatform = project.getExtensions()
            ...
            if (dependencies != null) {
                for (Node dependency : findChildren(dependencies, "dependency")) {
                    ...
                    if (versionProperty != null) {
                        findChild(dependency, "version").setValue(String.format("${%s}", versionProperty));
                    }
                }
            }
        }
            ...
            for (Library library : this.bom.getLibraries()) {
                for (Group group : library.getGroups()) {
                    ...
                    for (String pluginName : group.getPlugins()) {
                        ...
                        String value = (versionProperty != null) ? String.format("${%s}", versionProperty)
                                : library.getVersion();
